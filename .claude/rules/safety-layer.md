...
- `audit_schema_version: int = 1` — frozen at the constant in production code. Bump when the JSONL schema evolves; v0.2 readers gate on this. Currently `3` in production (bumped 1→2 by #54 for the `draft_skip_*` reasons, 2→3 by #55 for the `policy_hash` recipe change).
