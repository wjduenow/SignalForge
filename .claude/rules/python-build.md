...
The uv migration widened the CI matrix to 3.11 / 3.12 (uv's interpreter management makes a multi-version matrix essentially free). Issue #96 added 3.13 as the ceiling: Python 3.13 changed `Path.resolve()` to raise `OSError(errno.ELOOP)` instead of `RuntimeError` on cyclic symlinks (gh-108958), and `strict=False` resolution stopped raising on a cycle at all (it returns a best-effort unresolved path). The three loop-guard sites — `signalforge/_common/path_safety.py`, `signalforge/manifest/loader.py`, and `signalforge/demo/__init__.py` — now resolve the input path `strict=True` first (falling back to `strict=False` only on a genuinely missing target) and catch both `RuntimeError` and `OSError(errno.ELOOP)`, so symlink cycles fail loud on every supported version. Any new path-canonicalisation site MUST follow the same shape — a bare `except RuntimeError` silently passes loops through on 3.13.
...
`plans/super/1-project-scaffolding.md` — DEC-002, DEC-004, DEC-011, DEC-014. Issue #46 — Python version reconciliation. Issue #96 — Python 3.13 path-safety / ELOOP fix + matrix re-add. `plans/super/47-init-demo.md` — DEC-002 (Hatch `include` for non-`.py` package data), DEC-003 (`wheel_smoke` maintainer-gate pattern), DEC-006 (dotfile inclusion fallback).
