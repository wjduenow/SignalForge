...
|------|------------------|--------------------------|---------------------|
| `.signalforge/audit.jsonl` | One safety `AuditEvent` per `build_llm_request` call (LLM request leaving the warehouse boundary, plus column-name redaction map). Wide-table models may produce ≥ 2 JSONL lines per logical event correlated by `audit_id` (see [`safety-ops.md`](safety-ops.md) § "Audit JSONL schema (v4)"). | `audit_schema_version: 4` | `tests/safety/test_drift_detector.py` |
...
- **safety JSONL ↔ draft JSONL** — no shared content hash. The safety `AuditEvent` records the LLM-request envelope shape (mode, columns sent, `redactions_by_reason` + `column_name_map`, row count) but not SQL or any SQL hash; the draft `LLMResponseEvent` records `sent_sql_hash` over `Model.raw_code` plus the response-token economics. For cross-stage attribution within one `signalforge generate` invocation, group by `(model_unique_id, signalforge_version)` and order by `timestamp` — safety's audit fires before draft's in the same run. For wide-table models the safety audit may produce ≥ 2 JSONL lines per logical event, correlated by `audit_id`; reassemble with `signalforge.safety.audit.read_audit_events` before joining.
...
- **Column names** in safety records — every column the LLM sees in `schema-only` and `aggregate-only` modes is replaced with a stable `f"col_{blake2b(name.encode(), digest_size=4).hexdigest()}"` 8-hex-char hash (DEC-010 of [`safety-layer.md`](../.claude/rules/safety-layer.md)). The `(real → hashed)` map is recorded inside the safety JSONL's `column_name_map` field (paired with `redactions_by_reason` for the reason); the LLM only ever sees the hash. Use the audit log itself to map back. (Pre-#185 the same mapping lived in a per-record `redactions: tuple[RedactionRecord, ...]` array; v4 drops that field — see [`safety-ops.md`](safety-ops.md) § "Audit JSONL schema (v4)".)
- **Sample row values** in `sample` mode — every column that matched a policy opt-out signal (column meta, model meta, `tags: [pii]`, `meta.contains_pii`, or pattern match against `redact_patterns`) is recorded under its corresponding `RedactionReason` key in `redactions_by_reason`. The seven `RedactionReason` literals are documented in [`docs/safety-ops.md`](safety-ops.md) § "Per-column opt-out". (`RedactionRecord` the class is retained inside `signalforge.safety.request` as an internal value object on the build path; it no longer appears on `AuditEvent` post-#185.)
