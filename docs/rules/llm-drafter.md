...
When v0.2 swaps in OpenAI/Bedrock, the new vendor gets its own `_<vendor>_client.py` shim under `llm/` for the same reason. Don't pool SDK ignores into a generic util module.
