...
- **DEC-007 (QG finding — R1 non-orderable complex types, live-verified).** The QG correctness review found that R1's post-process assumed the aggregate always *returns* — but `MIN(map<…>)`/`MIN(variant)` fail at Spark **analysis** time (`INVALID_ORDERING_TYPE`), killing the whole query before the post-process runs. Confirmed live against the real warehouse. Fix: `column_stats` catches that specific `QuerySyntaxError` and re-runs a **reduced** aggregate (no MIN/MAX), returning `None` bounds — orderable complex types (`array`/`struct`/`binary`) keep the single-query post-process path; scalars unchanged. Live-verified (`column_stats(map)` → `min=max=None`, no crash). Committed under US-007. Lesson recorded in `warehouse-adapters.md` + memory: a Python post-process that rests on "the query succeeds and I'll fix up the result" needs live verification — a fake returning canned bounds can't disprove a query the warehouse rejects.
