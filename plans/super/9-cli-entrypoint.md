...
### Session 4 — 2026-06-05 (grade-completeness CLI surface — issue #202 US-007)

Issue #202 (DEC-204 + DEC-207) graduated `GradeConfig.require_complete`
and `GradeIncompleteError` in the grade engine (US-006, merged). US-007
surfaces that contract on the `generate` CLI with the no-clobber override
pattern. Captured as DEC-208 and folded into the `generate` flag set.

| ID | Decision | Rationale |
|---|---|---|
| **DEC-208** | Add `--require-complete` / `--no-require-complete` to `signalforge generate` via `argparse.BooleanOptionalAction` with **`default=None`** (the no-clobber sentinel). The override is applied in `_run_single_model` AFTER `load_grade_config(...)` ONLY when `args.require_complete is not None`, via `GradeConfig.model_validate({**grade_config.model_dump(), "require_complete": args.require_complete})` so validators re-run — mirroring the `--min-score` and prune `--scope` overlays. Precedence: explicit flag > `grade.require_complete` in `signalforge.yml` > library default (`True`). A bare run NEVER re-arms a `grade.require_complete: false` set in `signalforge.yml` (no CLI default clobbers the file value). When armed, the grade engine (US-006) raises `GradeIncompleteError` AFTER the fail-closed `grade.json` sidecar write if any non-exempt `(artifact, criterion)` pair stayed ungraded after the bounded transient-recovery sweep; the CLI's boundary catch maps it to **tier 2 / exit 2** and the stderr message names the still-ungraded pairs (header + named pairs, first ~20 then `… and N more`). Exempt (never trip): `"ceiling"` degrades and `"budget"` degrades when `total_budget_seconds` was set explicitly. Trip: `"transient"` degrades surviving the sweep and default-scaled-budget overruns (`total_budget_seconds` unset). Six parity surfaces updated in lockstep — argparse help, `cmd_generate` + `_run_single_model` docstrings, `docs/cli-ops.md` (flag reference + exit-code table + § Grade-completeness behaviour stderr shape), the CLI parity tests in `tests/cli/test_generate.py`, this DEC, and `src/signalforge/skills/signalforge/SKILL.md` (gated by `tests/cli/test_skill_cli_parity.py`). | The `default=None` sentinel is the load-bearing detail: the most common bug is a CLI default silently overwriting a file config. `BooleanOptionalAction` gives both `--flag` / `--no-flag` forms for free with a single sentinel. `model_validate` (not `model_copy`) re-runs the `extra="forbid"` validator so a future field-name drift fails loud. The CLI is a thin override-and-catch; the engine (US-006) owns the trip/exempt matrix and the raise-after-sidecar ordering. |
