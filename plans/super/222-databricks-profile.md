...
- **dbt-databricks target keys (reference).** dbt itself requires `host`, `http_path`, `schema`. **SignalForge's required-VALIDATED set is narrower** — `host` + `http_path` (always) + auth-conditional `token` (PAT) or `client_id`+`client_secret` (OAuth); `schema` (the shared `dataset` alias) and `catalog` are NOT in `_DATABRICKS_REQUIRED` (parse-optional, consistent with how Snowflake leaves `schema`/`database` optional). Auth: PAT (`token`) or OAuth-M2M (`auth_type: oauth` + `client_id` + `client_secret`). Optional: `catalog` (Unity Catalog), `threads`, `connect_*`. `host` is a workspace hostname (`dbc-xxxx.cloud.databricks.com`, no scheme); `http_path` is `/sql/1.0/warehouses/<id>` or `/sql/protocolv1/...`.
