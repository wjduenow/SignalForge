...
- **DEC-003 — Sample-mode behaviour (resolves Q-C; corrected by US-007a / tt8.15).** `_compile_row_count_between` always emits a CTE-wrapped failing-rows SELECT of the form `SELECT n FROM (SELECT COUNT(*) AS n FROM <table_ref> [WHERE <where>]) AS rc WHERE <bound-violation-predicate>` regardless of `prune.scope`. The bound-violation predicate is one of `n < <minimum>` (only minimum set), `n > <maximum>` (only maximum set), or `n < <minimum> OR n > <maximum>` (both set). Under `prune.scope="sample"` + `sample_strategy="materialised"` the `table_ref` is the temp table (correctly), so the count remains cheap. Under `prune.scope="sample"` + `sample_strategy="oneshot"` the bypass is deliberate — a sampled COUNT(*) is semantically wrong. Documented in `docs/prune-ops.md` § "row_count_between" + the variant docstring.
