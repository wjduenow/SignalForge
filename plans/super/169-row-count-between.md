...
- **DEC-003 — Sample-mode behaviour (resolves Q-C; corrected by US-007a / tt8.15; re-corrected by QG fix on tt8.13).** `_compile_row_count_between` always emits a CTE-wrapped failing-rows SELECT of the form `SELECT n FROM (SELECT COUNT(*) AS n FROM <table_ref> [WHERE <where>]) AS rc WHERE <bound-violation-predicate>` regardless of `prune.scope`. The bound-violation predicate is one of `n < <minimum>` (only minimum set), `n > <maximum>` (only maximum set), or `n < <minimum> OR n > <maximum>` (both set). **The engine's per-test loop overrides `table_ref` to the SOURCE table for `row_count_between` — never the materialised sample.** A COUNT(*) against a materialised sample returns the sample size (e.g. 100_000), not the model's real row count; bounds checked against sample size are semantically meaningless. The materialised-sample contract (#22) is correct for `not_null`/`unique`/`accepted_values`/`relationships`/`custom_sql` (which read row-level data the sample faithfully represents) but wrong for row-count primitives by definition. The COUNT(*) against the source is a single aggregate scan — cheap even on petabyte tables — so there's no cost argument for routing through the temp table. Under `prune.scope="sample"` + `sample_strategy="oneshot"` the bypass is also deliberate — a sampled COUNT(*) is semantically wrong by the same argument. Documented in `docs/prune-ops.md` § "Row-count cost model" + the `CandidateTestRowCountBetween` docstring + the engine's `per_test_table_ref` comment. Pinned by `test_prune_tests_row_count_between_under_materialised_references_source_not_temp_table`.

  **US-007a (tt8.15) correction.** US-007 originally landed the simpler `SELECT COUNT(*) FROM <table_ref> [WHERE <where>]` shape. The warehouse adapters wrap any compiler output as `SELECT COUNT(*) AS failures FROM (<sql>) AS t` (BigQueryAdapter line 942, SnowflakeAdapter line 852); under that wrap the original shape became `SELECT COUNT(*) AS failures FROM (SELECT COUNT(*) FROM <table>) AS t` — the inner returns 1 row (the count value), the outer `COUNT(*)` over a 1-row result is always 1, so `failures` was always 1 regardless of bounds. The engine's `failures==0 → always-passes` / `failures>0 → kept` matrix routed every real `row_count_between` to `kept` (or `failed-on-known-clean-data` on a trusted model) **without ever checking the bounds**. The CTE+WHERE shape pushes the bound check into the inner SELECT: when the bound holds the WHERE filters out the count row and the outer COUNT returns 0 (always-passes); when the bound is violated one row passes and the outer COUNT returns 1 (kept). The shape now matches the failing-rows-SELECT contract the other 4 built-in tests follow. No engine change required — the routing matrix is test-type-agnostic.
