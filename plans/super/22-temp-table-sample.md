...
- **R-OBS-3** — add a one-paragraph "audit reading guide" to `docs/prune-ops.md`: temp-table SQL of the form `FROM \`_SESSION._sf_sample_<run_id>\`` (two-part; the adapter returns `project=None` since BigQuery rejects the three-part `<project>._SESSION.<name>` form) is the materialisation signal in `compiled_sql`; oneshot runs continue to reference the source table directly.
...
- **DEC-001 — Compiled-SQL determinism via seeded run_id (R1=B).** `run_id = blake2b(table.qualified_name + signalforge_version + str(n) + canonical_json(partition_filter), digest_size=8).hexdigest()` (inputs joined with NUL separator; 16-hex output). Temp-table name = `_sf_sample_<run_id>`. Same input → byte-equal compiled SQL across runs; `compiled_sql_hash` invariant preserved unchanged. Rationale: simpler than snapshot normalisation; sessions provide namespace isolation so two concurrent runs on the same model don't collide on the temp-table identifier.

- **DEC-002 — BigQuery session lifecycle is adapter state (Option X internal).** `BigQueryAdapter` carries `self._active_session_id: str | None` for the duration of the prune run. `materialise_sample` runs the first query (`CREATE TEMP TABLE _sf_sample_<run_id> AS SELECT ...`) with `QueryJobConfig(create_session=True, ...)`; **BigQuery assigns the session_id server-side** and the SDK exposes it on the returned job via `job.session_info.session_id` — the adapter captures and stores that id. Subsequent `run_test_sql` calls use it via `connection_properties=[ConnectionProperty(key="session_id", value=...)]` so they can read `_SESSION._sf_sample_<run_id>`. **`BigQueryAdapter.__exit__` belt-and-braces closes the session explicitly via `CALL BQ.ABORT_SESSION()`** (DEC-013) — happy path runs, normal-error paths, and KeyboardInterrupt all fire `__exit__`. BigQuery's own session timeout (BQ-managed; ~24h max regardless of activity per DEC-013) is the durable fallback for hard process death (SIGKILL, OOM) where `__exit__` cannot fire. The ABC stays clean: no `session_id` kwarg on `run_test_sql`, no BQ-isms in `signalforge.prune`.

  **Note on `run_id` vs `session_id`:** these are TWO different identifiers and the plan keeps them distinct. `run_id` is OUR `blake2b(..., digest_size=8)` derivation (DEC-001) — it lands in the temp-table name `_sf_sample_<run_id>` for snapshot determinism. `session_id` is BQ's server-side assignment — it lands in `connection_properties` for query routing and in the DEC-014 WARNING. They are never interchangeable.
...
- **DEC-009 — Conservative-bias failure routing + degraded-run WARNING.** Any exception thrown by `adapter.materialise_sample(...)` (whether `MaterialisationFailedError`, `UnknownTableSizeError`, `SamplingRequiresPartitionFilterError`, or any other `WarehouseError` subclass) is caught at the orchestrator entry. Every candidate test routes to `kept-without-evidence` with the DEC-005 `why` shape; one `PruneEvent` per candidate is written to the audit JSONL (fail-closed audit preserved). **The orchestrator ALSO emits a single stderr WARNING** before iterating the candidate set, so the operator gets a one-line out-of-band signal that the run was degraded (otherwise the only signal is N identical `why` fields buried in the diff). Lazy-format JSON shape: `_LOGGER.warning("materialisation failed; routing all tests to kept-without-evidence: %s", json.dumps({"model_unique_id": ..., "candidate_count": N, "error_class": type(exc).__name__, "error_message": str(exc)[:200]}))`. Mirrors the existing v0.1 budget-exceeded WARNING pattern (`prune-engine.md` DEC-011 of #6).
...
- **DEC-013 — Explicit cleanup via `BQ.ABORT_SESSION()` in `__exit__`; BigQuery's own session timeout is the safety net.** `BigQueryAdapter.__exit__` checks `self._active_session_id`; if non-`None`, it issues `client.query("CALL BQ.ABORT_SESSION();", job_config=QueryJobConfig(connection_properties=[ConnectionProperty(key="session_id", value=self._active_session_id)]))` to immediately tear down the session and every `_SESSION.*` table inside it. Best-effort: any exception from the abort itself is **swallowed** (cleanup never blocks the user; their actual work succeeded). On success, one `INFO` log: `"session closed"` with `{"session_id_hash": ..., "ttl_remaining_seconds": ...}`. On failure, the DEC-014 cleanup-failure WARNING fires. `self._active_session_id = None` always set in a `finally` block so subsequent `__exit__` calls are no-ops.

  **TTL semantics — BQ-managed, not user-set.** BigQuery sessions have a server-enforced max lifetime (~24h regardless of activity) plus a BQ-default idle timeout. The `ttl_seconds` parameter on `materialise_sample(...)` is **NOT** a value we pass to BigQuery — it's a hint to OUR cleanup-WARNING text (the "auto-expire in Ns" line in DEC-014). We rely on BQ to enforce the actual TTL. This is documented in `docs/warehouse-adapter-ops.md` so a maintainer doesn't go looking for an SDK call that doesn't exist. v0.3 may revisit if BQ exposes a configurable per-session TTL knob; v0.2 trusts the BQ default.
...
- **DEC-014 — Cleanup-failure WARNING is operator-facing and contains the raw session_id.** When `BQ.ABORT_SESSION()` fails, the adapter emits a single `_LOGGER.warning(...)` with this exact multi-line shape (lazy %s positional, NOT f-string — passes the grep gate):
  ...
  The WARNING surfaces to stderr automatically via the CLI's `setup_logging`. Default level is INFO; `--quiet` raises the floor to WARNING (per `cli-layer.md`'s logging contract) — which means **`--quiet` does NOT suppress this WARNING**. That's deliberate: the cleanup-failure WARNING is operator-actionable (manual command + identifier inside), so we don't expose a path that silently drops it. If a future caller genuinely needs to silence everything (e.g., a notebook user testing in a tight loop), they can configure Python's logging directly — there's no CLI flag for it. No special CLI plumbing required for emission; Python's logging hierarchy handles it.
...
**Description.** Override `materialise_sample` on `BigQueryAdapter`. Compute `run_id = blake2b(table.qualified_name + signalforge_version + str(n) + canonical_json(partition_filter), digest_size=8).hexdigest()` (inputs joined with NUL separator; 16 hex chars) for snapshot determinism (DEC-001). Validate temp-table name `_sf_sample_<run_id>` via `validate_identifier`. Build `CREATE TEMP TABLE _sf_sample_<run_id> AS SELECT * FROM \`<source>\` WHERE MOD(ABS(FARM_FINGERPRINT(TO_JSON_STRING(t))), <bucket>) < 1 [AND <partition_filter>]` SQL (the CTAS uses the bare `_sf_sample_<run_id>` name — no `_SESSION.` prefix). Run via `client.query(sql, job_config=QueryJobConfig(create_session=True, ...defaults from _default_job_config(stage="warehouse_sample_materialise")))`. **BigQuery assigns the session_id server-side**; capture it from the returned `QueryJob` via `job.session_info.session_id` after `.result()` (the SDK populates `session_info` only after the job completes successfully). On success: store `self._active_session_id = <captured>`, `self._session_started_at = monotonic()`, `self._session_ttl_seconds = ttl_seconds` (the OUR-side hint per DEC-013, NOT a BQ knob); return `TableRef(project=None, dataset="_SESSION", name="_sf_sample_<run_id>")` (`project=None` is load-bearing — BigQuery rejects the three-part `<project>._SESSION.<name>` form, so the rendered `qualified_name` is the two-part `_SESSION._sf_sample_<run_id>`). On failure: wrap as `MaterialisationFailedError(cause=...)` — note that if the failure happens BEFORE `job.session_info` is populated, no session was ever created, so there's nothing to clean up; if it happens AFTER, `__exit__`'s swallow-and-warn path handles the orphan. Extend `run_test_sql` to honour `_active_session_id` when set (passes the same `connection_properties` so per-test queries can resolve `_SESSION._sf_sample_<run_id>`). Emit one `INFO` log via lazy-format JSON on success: `{"model": ..., "sample_rows": ..., "session_id_hash": blake2b-4(...), "run_id": ..., "duration_ms": ...}`. **Implement `__exit__` cleanup per DEC-013/DEC-014:** if `_active_session_id` is set, issue `CALL BQ.ABORT_SESSION()` in the same session; on success, `INFO` log `{"session_id_hash": ..., "ttl_remaining_seconds": ...}` and reset state; on failure, emit the DEC-014 multi-line WARNING (raw session_id + manual `bq query` command + "auto-expire in Ns" line driven by `_session_ttl_seconds`), swallow the exception, reset state in `finally`. Never log raw `session_id` outside the DEC-014 WARNING.
...
- `test_prune_tests_materialisation_failure_emits_orchestrator_warning` — pin DEC-009's degraded-run WARNING: when materialisation raises, capture log records and assert exactly one WARNING is emitted with the JSON payload `{"model_unique_id": ..., "candidate_count": N, "error_class": ..., "error_message": ...}`. Distinct from the per-decision `why` field (in-band signal) and from the cleanup-failure WARNING (DEC-014).
- `test_prune_tests_orchestrator_warning_fires_before_per_test_audit_writes` — assert log-record ordering: the WARNING fires once at the head of the failure path, BEFORE the N JSONL audit lines for the kept-without-evidence decisions.
...
- [ ] Materialisation runs before the per-test loop; failures route conservatively AND emit the DEC-009 degraded-run WARNING.
- [ ] `prune_tests` docstring documents the context-manager requirement: callers MUST invoke the adapter inside a `with` block; without it, materialised sessions rely on BigQuery's server-side timeout for cleanup (no explicit `BQ.ABORT_SESSION()` call). The CLI's `cmd_generate` already wraps the adapter; library callers (notebooks, scripts) are responsible.
- [ ] `tests/fixtures/prune/prune_event_v1.jsonl` regenerated to match the new `compiled_sql` content under `materialised`.
- [ ] All fifteen new tests pass.
...
6. **`docs/cli-ops.md` "Stderr shapes" section** — extend with explicit entries for the three distinct WARNING surfaces this work introduces / touches, so CI parsers and downstream tooling have a stable reference (preserves cli-layer.md's 5-surface parity rule):
   - **Cleanup-failure WARNING (DEC-014)** — multi-line, contains raw session_id + `bq query` command + "auto-expire in Ns" line. Fires from `BigQueryAdapter.__exit__`. NOT suppressed by `--quiet`.
   - **Materialisation-failure / degraded-run WARNING (DEC-009)** — single-line lazy-format JSON: `{"model_unique_id": ..., "candidate_count": N, "error_class": ..., "error_message": ...}`. Fires from `prune_tests` orchestrator at the head of the conservative-bias routing path.
   - **Budget-exceeded WARNING (existing v0.1, `prune-engine.md` DEC-011 of #6)** — pre-existing; documented here for parity so all three sit in one place.
...
2. **Seeded-determinism over snapshot normalisation.** DEC-001's `blake2b(stable_inputs, digest_size=8)` recipe for derived identifiers is a project pattern (mirrors the LLM drafter's `prompt_version`). Add a one-paragraph note to `.claude/rules/testing-signal.md` covering when to use seeded determinism vs snapshot normalisation.
