...
1. **`testing-signal.md` strict markers (lines 13-23)** — `addopts = "--strict-markers"` AND `strict_markers = true` BOTH required (pytest 9 quirk). New marker (if any) must be registered in `[tool.pytest.ini_options].markers`.
2. **`testing-signal.md` no-`assert True` (lines 7-11)** — Smoke test must be capable of failing if its target is broken. "Always-passes" smoke is worse than no smoke.
3. **`testing-signal.md` fixture regeneration (lines 29-37)** — Commit the generated manifest; document a `uvx`-pinned regen script; strip non-deterministic fields with `jq`. Pin the tool version in dev-deps for the latest schema only.
4. **`testing-signal.md` seeded determinism over normalisation (lines 40-41)** — Prefer hash-based stable identifiers over post-hoc regex normalisation in test assertions.
...
13. **`cli-layer.md` "Multi-surface parity" rule (lines 82-96)** — The README "Trying it out" section is one of the five user-facing surfaces. Update in lockstep with the test, the fixture path, the docstring on `_helpers.canonicalise_user_path`, and the DEC in the plan.
