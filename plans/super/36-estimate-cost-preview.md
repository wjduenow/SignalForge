...
| 9 | **API design — flag mutex** | pass | Add `--estimate` to existing `write_group` mutex at `generate.py:197`. argparse emits the standard mutex error and exits via `SystemExit(2)` directly; the `_EXCEPTION_TO_EXIT_CODE` table is not consulted (no `CliInputError` is raised). Exit 2 lines up with the four-tier taxonomy's input-validation tier (post-QG DRIFT-1 fix). |
...
| **DEC-007** | Stdout shape locked verbatim (snapshot-tested) | Three sections (Draft / Grade / Warehouse) + totals + footer with price-table version and tests/column heuristic. See the preview snapshot in this plan. Pinned by `tests/cli/test_estimate_render.py` against `tests/fixtures/estimate/output_happy.txt`. 80-col friendly. (Path corrected post-QG DRIFT-3.) |
| **DEC-008** | Add `--estimate` to existing `--write` / `--dry-run` mutex group | argparse emits the standard mutex error and exits via `SystemExit(2)` directly. No `CliInputError` is raised; the `_EXCEPTION_TO_EXIT_CODE` table is not consulted. Exit 2 matches the four-tier taxonomy's input-validation tier (post-QG DRIFT-1 fix). |
...
- `EstimateUnknownModelError` registered in `signalforge.cli._helpers._EXCEPTION_TO_EXIT_CODE` at tier 2 (input-validation — the operator's config picked a model we can't price). **Inheritance-vs-mapping precedence (post-CodeRabbit clarification):** `EstimateUnknownModelError` is an `LLMError` subclass but `map_exception_to_exit_code` walks `type(exc).__mro__` in order and looks up each class in the table — the concrete-class entry (tier 2) is found BEFORE the parent `LLMError` would be consulted, so the tier-2 mapping is deterministic and takes precedence over the broader LLM-tier-3 default. Same rule applies to any future `*Error` subclass whose intended tier differs from its parent: register the concrete class explicitly in the table; the MRO walk handles precedence.
...
- `signalforge generate --estimate --write <model>` exits 2 (argparse mutex; `SystemExit(2)` propagates directly — no `CliInputError` raised).
