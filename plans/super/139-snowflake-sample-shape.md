...
**Live Snowflake certification (DEC-006) — CERTIFIED 2026-05-27 ✅.** `test_snowflake_prune_live.py::test_prune_drops_always_passes_not_null_live_materialised_sample` ran green against a real Snowflake warehouse (password auth, writable schema; `SF_RUN_SNOWFLAKE=1`, `1 passed in 14.78s`): the `scope="sample"` + `materialised` path executed the `CREATE TEMPORARY TABLE … AS SELECT * EXCLUDE (_sf_sample_hash) …` CTAS and the always-passes `not_null` was dropped. This **resolves DEC-004 in favour of the PRIMARY form** — Snowflake accepts `ORDER BY _sf_sample_hash` even though that column is `SELECT * EXCLUDE`-d, so the documented fallback (drop the outer `ORDER BY`) is **not** needed and the shipped fixtures stand. The offline tiers (snapshots + sqlglot) certified shape; this live run certified Snowflake acceptance — the merge gate is satisfied.
