"""Hand-rolled fake for a ``databricks.sql`` connection (US-002, #224).

Mirrors :mod:`tests.warehouse._fake_snowflake` (the Snowflake fake), which in
turn mirrors the structure / quality bar of ``tests/warehouse/_fake.py`` (the
...
The fake satisfies the duck-typed protocols the (later-bead) DatabricksAdapter
consumes:

* :class:`FakeDatabricksConnection` satisfies
  :class:`signalforge.warehouse.adapters._databricks_client._DatabricksClientProtocol`
  (``cursor()`` / ``close()``).
* :class:`_FakeDatabricksCursor` satisfies
  :class:`signalforge.warehouse.adapters._databricks_client._DatabricksCursorProtocol`
  (``execute()`` / ``fetchall()`` / ``close()`` / ``description``).

Tests register query round-trips via
:meth:`FakeDatabricksConnection.expect_execute`; each ``cursor().execute(sql)``
consumes one matching expectation. Calls outside the canned set raise
``AssertionError("unexpected query: ...")`` (mirrors the BigQuery / Snowflake
fakes' loud posture). Connection cleanup is observable via
:attr:`close_call_count`, and a ``close_raises`` kwarg drives the
:meth:`DatabricksAdapter._cleanup_active_session` swallow-and-warn path. The
adapter reads ``conn.session_id`` for the cleanup log, so the fake exposes an
injectable :attr:`session_id`.
...
class _ExecuteExpectation:
    ...
    the next :meth:`_FakeDatabricksCursor.fetchall`) or an :class:`Exception`
...
class _FakeDatabricksCursor:
    """Stand-in for a ``databricks.sql`` cursor.

    Bound to its parent :class:`FakeDatabricksConnection` so a single
    ...
    def __init__(self, connection: FakeDatabricksConnection) -> None:
...
class FakeDatabricksConnection:
    ...
    Args:
        session_id: Opaque session-id string the adapter reads via
            ``conn.session_id`` for the cleanup log. Defaults to a fixed fake
            id.
        close_raises: When supplied, :meth:`close` raises this exception — this
            drives :meth:`DatabricksAdapter._cleanup_active_session`'s
