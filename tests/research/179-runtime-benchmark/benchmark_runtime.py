...
def _grade_degradations(grade_data: dict) -> tuple[int, int, int]:
    ...
    * ``degraded_budget`` — the subset whose ``reasoning`` names a budget
      timeout (the 2026-05-30 baseline's 17/34 failure mode). The number
      #186/#187 must drive down.
    """
    comparable = degraded_total = degraded_budget = 0
    for result in grade_data.get("results", []):
        if result.get("score") is None:
            degraded_total += 1
            if "budget" in (result.get("reasoning") or "").lower():
                degraded_budget += 1
        else:
            comparable += 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    if returncode != 0:
        ...
        # sidecar BEFORE raising a below-threshold error (DEC-021), so the
        # timing sidecars may still be present and valid. Surface it, keep going.
        print(f"  NOTE: generate exited {returncode} — parsing sidecars anyway")
