...
# ---------------------------------------------------------------------------
# #154 US-004: manifest-ingested custom_sql (from_manifest=True). The body is
# dbt's already-Jinja-resolved compiled_code referencing dbt's OWN quoted
# relation. DEC-007 — full-scope against the source, as-is (NEVER a
# _SESSION._sf_sample_* temp table), even when scope=sample is passed.
# DEC-013 — comment-tolerant validation (dbt SQL carries -- / /* */ comments).
# DEC-012 — determinism belt-and-braces fallback in the compiler.
# ---------------------------------------------------------------------------

# A realistic dbt-compiled body: dbt quotes the relation with backticks, which
# neither own_qualified (unquoted) nor own_table_quoted would match — the exact
# shape the ingested full-scope path exists for (#154 AR row 8).
_INGESTED_COMPILED_SQL = "select order_id\nfrom `fake_project`.`dataset`.`orders`\nwhere total < 0"


def test_compile_ingested_custom_sql_full_returns_body_as_is() -> None:
    """A manifest-ingested custom_sql (from_manifest=True) compiles to its
    compiled_code body unchanged — the adapter wraps the COUNT(*) envelope
    like every other variant. dbt's own quoted relation is preserved."""
...
def test_compile_ingested_custom_sql_comment_bearing_body_compiles_comment_free() -> None:
    """#270 DEC-001 (G3): a compiled body carrying dbt's -- line comments and
    /* */ block comments compiles to a comment-FREE body. The adapter's
    execution-time :func:`validate_test_sql` is comment-INTOLERANT, so the
    compiler strips comments here (rather than relaxing the strict validator);
    the returned body then passes ``validate_test_sql`` — the common
    comment-bearing dbt body prunes end-to-end instead of never pruning."""
    body = (
    ...
    assert isinstance(actual, str)
    # The output is comment-free: neither comment marker survives.
    assert "--" not in actual
    assert "/*" not in actual
    assert "*/" not in actual
    # It equals the stripped body (validate≡execute — what we return is what runs).
    assert actual == strip_sql_comments(body)
    # And the load-bearing property: the comment-free body passes the adapter's
    # comment-INTOLERANT execution-time validator (which raises on -- / /* */).
    validate_test_sql(actual)


def test_compile_ingested_custom_sql_non_deterministic_returns_sentinel() -> None:
    """DEC-012 belt-and-braces: a non-deterministic ingested body reaching the
    compiler (the ingest bridge is the primary gate) routes to
    _InvalidIdentifier → kept-without-evidence. A run-to-run-varying verdict
    would violate Architectural Commitment #5."""
    body = (
        "select order_id\n"
        "from `fake_project`.`dataset`.`orders`\n"
...
def test_compile_ingested_custom_sql_injection_returns_sentinel() -> None:
    """A genuine injection (a top-level ``;`` outside any comment/string) still
    fails loud through the comment-tolerant scan → _InvalidIdentifier."""
    body = "select order_id from `fake_project`.`dataset`.`orders`; drop table x"
    test = CandidateTestCustomSQL(sql=body, from_manifest=True)
    result = _compile_test(
        test,
        _make_orders_table_ref(),
...
def test_compile_ingested_count_scalar_restructured_to_failing_rows() -> None:
    ...
    assert isinstance(actual, str)
    # The exact failing-rows scalar-subquery restructure. #270 DEC-001a puts a
    # ``\n`` before the closing suffix so trailing body content (e.g. a ``-- x``
    # line comment) can never swallow ``) AS sf_agg_value) AS sf_agg WHERE …``.
    assert actual == (
        "SELECT sf_agg_value FROM (SELECT (\n"
        f"{_INGESTED_COUNT_SCALAR_SQL}\n"
...
# ---------------------------------------------------------------------------
# #270 US-003 — the three compiler wiring changes: comment-strip (DEC-001),
# dialect-refusal (DEC-003), and the count-compose newline (DEC-001a, the §1.4
# splice bug).
# ---------------------------------------------------------------------------


def test_compile_ingested_count_scalar_trailing_line_comment_splice_is_well_formed() -> None:
    """The §1.4 splice bug: a count-scalar body ending in a ``-- tail`` line
    comment must NOT comment out ``) AS sf_agg_value) AS sf_agg WHERE …``.

    Two independent defences (#270 DEC-001 + DEC-001a) apply here: (1) the body is
    comment-STRIPPED before compose, so the ``-- tail`` is gone; (2) the compose
    puts a ``\\n`` before the closing suffix so a single-line body physically
    cannot swallow it. The result is a well-formed restructure that parses and
    passes the adapter's comment-intolerant validator."""
    body = (
        "select count(*)\n"
        "from `fake_project`.`dataset`.`orders`\n"
        "where total < 0  -- negative totals are failing rows"
    ...
    assert isinstance(actual, str)
    # The trailing comment is gone; the failing-rows suffix survives on its own line.
    assert "--" not in actual
    assert actual.endswith(") AS sf_agg_value) AS sf_agg WHERE sf_agg_value <> 0")
    # The stripped body is the compose subject.
    stripped = strip_sql_comments(body)
    assert actual == (
        "SELECT sf_agg_value FROM (SELECT (\n"
        f"{stripped}\n"
        ") AS sf_agg_value) AS sf_agg WHERE sf_agg_value <> 0"
    )
    # It is well-formed SQL the adapter's comment-intolerant validator accepts.
    validate_test_sql(actual)


def test_compile_ingested_custom_sql_unparseable_under_live_dialect_returns_sentinel() -> None:
    """#270 DEC-003 (G1): a body that parses under the ingest-side ``"bigquery"``
    default but NOT under the LIVE warehouse dialect is refused at the compiler —
    ``_InvalidIdentifier`` → ``kept-without-evidence`` — BEFORE the always-1
    verbatim wrap. Here a Snowflake-invalid backtick-quoted relation (BigQuery
    accepts backticks; Snowflake does not) compiled under ``SNOWFLAKE_DIALECT``."""
    # BigQuery parses this; Snowflake rejects the backtick quoting.
    ...
    # Sanity: the SAME body compiles fine under BigQuery (proves it is the DIALECT
    # that refuses, not the body being intrinsically bad).
    ok = _compile_test(
...
def test_compile_ingested_custom_sql_dialect_refusal_precedes_determinism_gate() -> None:
    """The dialect-refusal gate is FIRST — a body that is BOTH unparseable under
    the live dialect AND non-deterministic surfaces the DIALECT reason (the
    correctness gate that closes G1), not the determinism reason."""
    # Backtick relation (Snowflake-invalid) + a time-dependent function.
...
def test_compile_ingested_custom_sql_override_relation_rewritten_and_comment_stripped() -> None:
    """The override splice preserves comments BYTE-INTACT (DEC-015 — a
    character-offset op on the identical ``str`` object; verified inside
    :func:`_verified_ingested_override`), but the compiler then STRIPS them
    (#270 DEC-001) because the adapter's execution-time ``validate_test_sql`` is
    comment-INTOLERANT. So the compiler's returned string carries the temp-table
    relation rewrite AND is comment-free — and passes the strict validator. The
    override is a COMPLETE string (already spliced+verified on the unstripped body),
    so stripping it here does no span operation and cannot desync #268's offsets."""
    body = (
        "-- generated by dbt-expectations\n"
        "select\n"
        "    order_id\n"
    ...
    # The override the engine threads in DOES carry the comments byte-intact.
    assert "-- generated by dbt-expectations" in override
    assert "/* the orders relation */" in override
    ...
    assert isinstance(actual, str)
    # The compiler returns the comment-STRIPPED override.
    assert actual == strip_sql_comments(override)
    assert "--" not in actual
    assert "/*" not in actual
    assert "*/" not in actual
    # The temp-table relation rewrite survives; not one byte of the source relation.
    assert "`_SESSION._sf_sample_deadbeefcafef00d`" in actual
    assert "orders" not in actual
    assert "fake_project" not in actual
    # And the stripped override passes the adapter's comment-intolerant validator.
    validate_test_sql(actual)
...
def test_compile_drafted_custom_sql_still_samples_unchanged() -> None:
    ...
    {{ this }} substitution + deterministic-sample CTE. The ingested path
    must not alter drafted-custom_sql routing."""
    expected = _read_fixture("custom_sql_sample.sql")
    drafted = CandidateTestCustomSQL(sql="select order_id from {{ this }} where total < 0")
