...
from signalforge.ingest._compiled_sql import strip_sql_comments
...
def test_prune_tests_ingested_comment_bearing_body_audit_records_stripped_bytes(
    tmp_path: Path,
) -> None:
    """#270 US-003 DEC-001 (G3): a comment-bearing manifest body prunes
    end-to-end — the compiler strips comments so the adapter's
    comment-INTOLERANT ``validate_test_sql`` accepts it — and both the in-memory
    ``PruneDecision.compiled_sql`` AND the durable audit ``compiled_sql`` record
    the STRIPPED bytes (what actually ran), NOT the raw comment-bearing body.
    The raw ``compiled_code`` survives verbatim in the operator's manifest."""
    audit_path = tmp_path / "prune.jsonl"
    fake = FakeBigQueryClient(project="fake_project")
    fake.expect_query(matching=r"SELECT COUNT\(\*\)", returns=[{"failures": 0}])
    adapter = _make_adapter(fake)
    ...
    body = (
        ...
        "select id\n"
        "from `fake_project`.`dataset`.`orders`  /* orders */\n"
        "where status = 'BAD'  -- flagged rows"
    )
    model = _make_orders_model()
    manifest = _make_manifest(model)
    ...
    decision = result.decisions[0]
    # It pruned — the comment-bearing body was NOT false-rejected.
    assert decision.decision == "dropped"
    assert decision.reason == "always-passes"
    # The recorded compiled_sql is comment-FREE (what ran).
    assert "--" not in decision.compiled_sql
    assert "/*" not in decision.compiled_sql
    assert decision.compiled_sql == strip_sql_comments(body)
    # The durable audit records the same stripped bytes.
    audit_rows = _read_audit_lines(audit_path)
    assert len(audit_rows) == 1
    assert audit_rows[0]["compiled_sql"] == strip_sql_comments(body)
    fake.assert_all_expectations_met()
...
# ``SELECT sf_agg_value FROM (SELECT (\n<body>\n) AS sf_agg_value) AS sf_agg
# WHERE sf_agg_value <> 0`` (#270 DEC-001a puts a ``\n`` before the closing
# suffix so a trailing body comment can never swallow it) so the adapter's
